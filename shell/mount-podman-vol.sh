#!/bin/sh
#
#  This script mounts a Podman volume at a directory so that container
...
VOL_DIR=$(podman volume inspect --format '{{.MountPoint}}' "$1") \
    || usage "Volume '$1' not found"

# Determine UID and GID of the volume owner
VOL_UID=$(stat --format '%u' $VOL_DIR)
VOL_GID=$(stat --format '%g' $VOL_DIR)
...
echo "Podman volume '$1' now mounted at directory $MAPPED_DIR"
